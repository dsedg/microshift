...
if [ ! -d "${GO_INSTALL_DIR}" ]; then
    ...
    mkdir -p "/usr/local/go${GO_VER}"
    tar -C "/usr/local/go${GO_VER}" -xzf "go${GO_VER}.linux-${GO_ARCH}.tar.gz" --strip-components 1
    rm -rfv /usr/local/bin/{go,gofmt}
    ln --symbolic /usr/local/go${GO_VER}/bin/{go,gofmt} /usr/local/bin/
